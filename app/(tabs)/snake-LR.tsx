...
const SnakeLR = () => {
  ...
  const playRound = async () => {
    ...
    const randomWait = Math.floor(Math.random() * 11 + 3) * 1000;
    await delay(randomWait);

    // Word selection phase
    const randomValue = Math.random();
    const selectedWord = 
      randomValue < 0.5 ? 'left' : 'right';
    ...
    if (isRunningRef.current) {
      timeoutRef.current = setTimeout(playRound, 6000);
    }
  };
  ...
  return (
    <ParallaxScrollView
      ...
      <ThemedView style={styles.titleContainer}>
        <ThemedText type="title">SnakeLR</ThemedText>
      </ThemedView>
